...
static void object_available_cb(GUPnPDIDLLiteParser *parser, GUPnPDIDLLiteObject *object, Browser *browser) {
    ...
    if (g_strcmp0(object_id, browser->current_item->object_id) == 0) {
        // The object is the current item, update its metadata

        item_set_metadata(
            browser->current_item,
            parent_id,
            gupnp_didl_lite_object_get_title(object),
            gupnp_didl_lite_object_get_upnp_class(object)
        );

        gtk_window_set_title(GTK_WINDOW(browser->window), browser->current_item->title);
        gtk_widget_set_sensitive(browser->up_button, TRUE);
    } else if (g_strcmp0(parent_id, browser->current_item->object_id) == 0) {
        // The object is a child of the current item, add it to the list

        Item *child = item_new(browser->current_item->content_directory, object_id);
        item_set_metadata(
            child,
            parent_id,
            gupnp_didl_lite_object_get_title(object),
        ...
        g_list_store_append(browser->items, child);
        g_object_unref(child);
    }
}

static void device_proxy_available_cb(GUPnPControlPoint *cp, GUPnPDeviceProxy *proxy, Browser *browser) {
    if (browser->current_item->content_directory == NULL) {
        ...
        if (content_directory) {
            ...
            g_list_store_append(browser->items, item);
            g_object_unref(content_directory);
        }
    }
}

static void setup_item_cb(GtkSignalListItemFactory *factory, GtkListItem *list_item, Browser *browser) {
...
static void bind_item_cb(GtkSignalListItemFactory *factory, GtkListItem *list_item, Browser *browser) {
    GtkWidget *box = gtk_list_item_get_child(list_item);
    Item *item = gtk_list_item_get_item(list_item);
    
    GtkWidget *icon = gtk_widget_get_first_child(box);
    if (browser->current_item->content_directory) {
...
static void activate_item_cb(GtkGridView *grid_view, guint position, Browser *browser) {
    Item *item = g_list_model_get_item(G_LIST_MODEL(browser->items), position);

    if (g_str_has_prefix(item->upnp_class, "object.container")) {
        browse(browser, item->content_directory, item->object_id);
...
static void up_button_clicked_cb(GtkButton *button, Browser *browser) {
    if (browser->current_item->parent_id) {
        if (g_strcmp0(browser->current_item->parent_id, "-1") == 0) {
            // The current item is the root, go back to media servers list

            g_object_unref(browser->current_item);
            browser->current_item = item_new(NULL, "-1");
            g_list_store_remove_all(browser->items);

            gssdp_resource_browser_set_active(GSSDP_RESOURCE_BROWSER(browser->cp), FALSE);
            gssdp_resource_browser_set_active(GSSDP_RESOURCE_BROWSER(browser->cp), TRUE);

            gtk_window_set_title(GTK_WINDOW(browser->window), "Media Servers");
            gtk_widget_set_sensitive(browser->up_button, FALSE);
        } else {
            // browse replaces the current item, so we need to keep a reference to it
            Item *old_current = g_object_ref(browser->current_item);

            browse(browser, old_current->content_directory, old_current->parent_id);
...
static void activate_cb(GtkApplication *app, Browser *browser) {
    GError *error = NULL;

    browser->window = gtk_application_window_new(app);
    gtk_window_set_title(GTK_WINDOW(browser->window), "Media Servers");
    gtk_window_set_default_size(GTK_WINDOW(browser->window), 800, 600);

    GtkWidget *header_bar = gtk_header_bar_new();
    gtk_header_bar_set_show_title_buttons(GTK_HEADER_BAR(header_bar), TRUE);
    gtk_window_set_titlebar(GTK_WINDOW(browser->window), header_bar);

    browser->up_button = gtk_button_new_from_icon_name("go-up-symbolic");
    gtk_widget_set_sensitive(browser->up_button, FALSE);
    g_signal_connect(browser->up_button, "clicked", G_CALLBACK(up_button_clicked_cb), browser);
    gtk_header_bar_pack_start(GTK_HEADER_BAR(header_bar), browser->up_button);

    GtkWidget *scrolled_window = gtk_scrolled_window_new();
    gtk_widget_set_vexpand(scrolled_window, TRUE);
    gtk_widget_set_hexpand(scrolled_window, TRUE);
    gtk_window_set_child(GTK_WINDOW(browser->window), scrolled_window);

    browser->items = g_list_store_new(ITEM_TYPE);
    GtkSingleSelection *model = gtk_single_selection_new(G_LIST_MODEL(browser->items));

    GtkListItemFactory *factory = gtk_signal_list_item_factory_new();
    g_signal_connect(factory, "setup", G_CALLBACK(setup_item_cb), browser);
    g_signal_connect(factory, "bind", G_CALLBACK(bind_item_cb), browser);

    GtkWidget *grid_view = gtk_grid_view_new(GTK_SELECTION_MODEL(model), factory);
    g_signal_connect(grid_view, "activate", G_CALLBACK(activate_item_cb), browser);
    ...
    browser->parser = gupnp_didl_lite_parser_new();
    g_signal_connect(browser->parser, "object-available", G_CALLBACK(object_available_cb), browser);

    browser->cp = gupnp_control_point_new(context, "urn:schemas-upnp-org:device:MediaServer:1");
    g_signal_connect(browser->cp, "device-proxy-available", G_CALLBACK(device_proxy_available_cb), browser);
    gssdp_resource_browser_set_active(GSSDP_RESOURCE_BROWSER(browser->cp), TRUE);
...
int main(int argc, char **argv) {
    ...
    g_signal_connect(app, "activate", G_CALLBACK(activate_cb), &browser);
